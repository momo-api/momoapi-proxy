...
test("checkLatestVersion detects updates from CDN JSON payload", async () => {
  ...
  const fakeFetch = async (url) => {
    if (url.includes("bridge-latest.json")) {
      return new Response(JSON.stringify({
        version: "0.13.7",
        url: "https://momoapi.us/install/packages/momoapi-proxy-0.13.7.tgz",
  ...
  assert.equal(info.latest, "0.13.7");
  assert.equal(info.hasUpdate, true);
  assert.equal(info.downloadUrl, "https://momoapi.us/install/packages/momoapi-proxy-0.13.7.tgz");
});
  ...
  try {
    staged = await updateSelf({
      ...
      fetchImpl: async (url) => {
        if (url.includes("bridge-latest.json")) {
          return new Response(JSON.stringify({
            version: "0.13.7", url: "https://momoapi.us/install/packages/momoapi-proxy-0.13.7.tgz",
          }), { status: 200 });
        }
        if (url.includes("api.github.com")) {
          return new Response(JSON.stringify({
    ...
    assert.equal(staged.previous, "0.13.6");
    assert.equal(staged.current, "0.13.7");
    assert.equal(JSON.parse(readFileSync(join(staged.rootDir, "package.json"), "utf8")).version, "0.13.6");
    assert.equal(JSON.parse(readFileSync(join(staged.stagingDir, "package.json"), "utf8")).version, "0.13.7");
