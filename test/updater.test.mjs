...
test("checkLatestVersion detects updates from CDN JSON payload", async () => {
  ...
  const fakeFetch = async (url) => {
    if (url.includes("bridge-latest.json")) {
      return new Response(JSON.stringify({
        version: "0.13.9",
        url: "https://momoapi.us/install/packages/momoapi-proxy-0.13.9.tgz",
    ...
    if (url.includes("api.github.com")) {
      return new Response(JSON.stringify({
        tag_name: "v0.13.9",
        assets: [{
          name: "momoapi-proxy-0.13.9.tgz",
          browser_download_url: "https://github.com/momo-api/momoapi-proxy/releases/download/v0.13.9/momoapi-proxy-0.13.9.tgz",
  ...
  assert.equal(info.latest, "0.13.9");
  assert.equal(info.hasUpdate, true);
  assert.equal(info.downloadUrl, "https://momoapi.us/install/packages/momoapi-proxy-0.13.9.tgz");
});
...
test("self-update accepts GitHub's signed release-asset redirect", async () => {
  ...
  const archive = join(fixture, "momoapi-proxy-0.13.9.tgz");
  const proxyHome = join(fixture, "home");
  mkdirSync(join(source, "bin"), { recursive: true });
  mkdirSync(join(source, "src"), { recursive: true });
  writeFileSync(join(source, "package.json"), JSON.stringify({ version: "0.13.9" }));
  ...
  try {
    ...
    assert.equal(staged.current, "0.13.9");
  } finally {
  ...
  try {
    staged = await updateSelf({
      ...
      fetchImpl: async (url) => {
        if (url.includes("bridge-latest.json")) {
          return new Response(JSON.stringify({
            version: "0.13.9", url: "https://momoapi.us/install/packages/momoapi-proxy-0.13.9.tgz",
          }), { status: 200 });
        }
        if (url.includes("api.github.com")) {
          return new Response(JSON.stringify({
    ...
    assert.equal(staged.previous, "0.13.8");
    assert.equal(staged.current, "0.13.9");
    assert.equal(JSON.parse(readFileSync(join(staged.rootDir, "package.json"), "utf8")).version, "0.13.8");
    assert.equal(JSON.parse(readFileSync(join(staged.stagingDir, "package.json"), "utf8")).version, "0.13.9");
