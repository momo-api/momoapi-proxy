...
test("checkLatestVersion detects updates from CDN JSON payload", async () => {
  ...
  const fakeFetch = async (url) => {
    if (url.includes("bridge-latest.json")) {
      return new Response(JSON.stringify({
        version: "0.13.2",
        url: "https://momoapi.us/install/packages/momoapi-proxy-0.13.2.tgz",
    ...
    if (url.includes("api.github.com")) {
      return new Response(JSON.stringify({
        tag_name: "v0.13.2",
        assets: [{
          name: "momoapi-proxy-0.13.2.tgz",
          browser_download_url: "https://github.com/momo-api/momoapi-proxy/releases/download/v0.13.2/momoapi-proxy-0.13.2.tgz",
  ...
  assert.equal(info.latest, "0.13.2");
  assert.equal(info.hasUpdate, true);
  assert.equal(info.downloadUrl, "https://momoapi.us/install/packages/momoapi-proxy-0.13.2.tgz");
});
