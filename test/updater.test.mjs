...
test("checkLatestVersion detects updates from CDN JSON payload", async () => {
  ...
  const fakeFetch = async (url) => {
    if (url.includes("bridge-latest.json")) {
      return new Response(JSON.stringify({
        version: "999.0.0",
        url: "https://momoapi.us/install/packages/momoapi-proxy-999.0.0.tgz",
    ...
    if (url.includes("api.github.com")) {
      return new Response(JSON.stringify({
        tag_name: "v999.0.0",
        assets: [{
          name: "momoapi-proxy-999.0.0.tgz",
          browser_download_url: "https://github.com/momo-api/momoapi-proxy/releases/download/v999.0.0/momoapi-proxy-999.0.0.tgz",
  ...
  assert.equal(info.downloadUrl, "https://momoapi.us/install/packages/momoapi-proxy-999.0.0.tgz");
});
...
test("self-update accepts GitHub's signed release-asset redirect", async () => {
  ...
  const archive = join(fixture, "momoapi-proxy-999.0.0.tgz");
  const proxyHome = join(fixture, "home");
  mkdirSync(join(source, "bin"), { recursive: true });
  mkdirSync(join(source, "src"), { recursive: true });
  writeFileSync(join(source, "package.json"), JSON.stringify({ version: "999.0.0" }));
  ...
  try {
    ...
    assert.equal(staged.current, "999.0.0");
  } finally {
  ...
  try {
    staged = await updateSelf({
      ...
      fetchImpl: async (url) => {
        if (url.includes("bridge-latest.json")) {
          return new Response(JSON.stringify({
            version: "999.0.0", url: "https://momoapi.us/install/packages/momoapi-proxy-999.0.0.tgz",
          }), { status: 200 });
        }
        if (url.includes("api.github.com")) {
          return new Response(JSON.stringify({
    ...
    assert.equal(staged.previous, getCurrentVersion());
    assert.equal(staged.current, "999.0.0");
    assert.equal(JSON.parse(readFileSync(join(staged.rootDir, "package.json"), "utf8")).version, getCurrentVersion());
    assert.equal(JSON.parse(readFileSync(join(staged.stagingDir, "package.json"), "utf8")).version, "999.0.0");
