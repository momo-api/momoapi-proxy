...
截至 `5dc073c`，`server.mjs` 已从 P5 前基线约 2,644 行降至 1,234 行，减少约 1,410 行；这只是维护性指标，不代表端到端性能自动提升。HTTP 生命周期边界已完成，当前仍需继续拆分 compact 及路由编排边界，并为每个切片保持 wire、tool-call、背压和取消回归。P5 未完成，尚未进入 P6 发布。
