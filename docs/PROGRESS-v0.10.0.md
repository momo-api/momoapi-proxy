...
- **状态**: ✅ v0.10.2 代码整改完成，57 项测试通过，等待发布产物校验
- **目标发布版本**: `v0.10.2`
- **最新更新**: 2026-09-07 13:35
...
| **P0-2** | 真正的 Graceful Shutdown 与 Draining | Node Core (`server.mjs`) | ✅ **已完成** | shutdown 响应结束后立即 `server.close()`，新 TCP 连接被拒绝；既有请求允许自然完成；截止时间到达后发送结构化 `response.incomplete`、abort 上游并销毁残留 socket；退出依赖可注入，测试不再提前结束进程 |
...
| **Tool-Fix** | Gemini 3.8 自定义工具协议适配与参数兼容 | Tool Bridge (`tools.mjs`, `server.mjs`, `responses-compat.mjs`) | ✅ **已完成** | `functions` 内建命名空间保持裸名 `exec`；支持 `input/raw/command/cmd`；裸命令生成 `tools.exec_command({ cmd })`；测试会真实执行生成的 JavaScript 并验证参数对象，不再只匹配字符串 |

---

## 自动化测试与三端镜像校验 (QA & Distribution Verification)

- **全量测试套件**: **57 项单元/集成测试全部通过** (57 passed, 0 failed, 0 skipped)。
  ...
  - `test/graceful-shutdown.test.mjs` (2/2 passed)
  - `test/gemini-custom-tool.test.mjs` (4/4 passed)
  - 核心桥接与兼容测试 (43/43 passed)
- **三端公开下载校验**: 发布 `v0.10.2` 后填写最终 SHA256；禁止再移动旧 `v0.10.1` Tag。
