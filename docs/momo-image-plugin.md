...
| gpt-image-2.5-sunburst | POST /v1/images/generations | Same endpoint with image_urls (public URL strings) | 1-4 | 1-16 | Native quality, arbitrary valid WIDTHxHEIGHT, format, compression, background, moderation; no streaming partial images |
| gpt-image-2.5-flare | POST /v1/images/generations | Same endpoint with image_urls (public URL strings) | 1-4 | 1-16 | Same native controls; optimized for faster everyday generation |
...
Mask input remains disabled for the three legacy routes. APIMart GPT Image 2.5 uses quality auto/low/medium/high/xhigh/max, resolution 1k/2k/4k, output format png/jpeg/webp, output compression 0-100 for JPEG/WebP, background auto/opaque/transparent, and moderation auto/low. APIMart does not document mask, input_fidelity, stream, or partial_images; the adapter rejects those controls. Custom dimensions must use multiples of 16, stay between 1:3 and 3:1, keep each edge at or below 3840 pixels, and contain 655,360-8,294,400 total pixels.

For GPT Image 2.5, an edit is sent as JSON to POST /v1/images/generations using image_urls: string[]. APIMart requires publicly reachable HTTP(S) URLs and accepts at most 16 references. HTTPS references are validated and forwarded without downloading. If an input is an asset reference or data URL, the proxy first uploads it to POST /v1/uploads/images (multipart field file), then sends the temporary public URL in image_urls. APIMart does not use /v1/images/edits, images[].image_url, or multipart image fields for this model.
