...
For GPT Image 2.5, an edit is sent as JSON to POST /v1/images/generations using image_urls: string[]. APIMart accepts public HTTP(S) URLs or image data URLs and accepts at most 16 references. HTTPS references are validated and forwarded without downloading. Asset references are resolved to image data URLs locally and sent directly; the MOMO gateway does not expose /v1/uploads/images. APIMart does not use /v1/images/edits, images[].image_url, or multipart image fields for this model.
