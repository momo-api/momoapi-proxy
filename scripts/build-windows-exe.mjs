...
if (existsSync(ICO_PATH)) {
  ...
  try {
    execSync(`npx --yes resedit-cli --in "${exeOutput}" --out "${exeOutput}" --ignore-signed --icon "1,${ICO_PATH}" --product-name "MOMO API Proxy" --file-description "MOMO API Proxy for Codex & Desktop AI" --company-name "MOMO API" --file-version "0.9.5.0" --product-version "0.9.5.0" --original-filename "momoapi-proxy-windows-x64.exe"`, {
      cwd: ROOT,
      stdio: "inherit",
    });
