...
export async function runImageMcp() {
  ...
  for await (const line of rl) {
    ...
    if (request.method === "initialize") {
      process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result: { protocolVersion: request.params?.protocolVersion || "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "momo-image", version: "0.6.2" } } }) + "\n");
      continue;
    }
    try {
